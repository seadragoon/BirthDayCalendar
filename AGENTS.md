...
| 18 | セキュリティ強化（4桁PINパスコード・生体認証・タスクスイッチャー目隠し・自動再ロック） | ✅ 完了 |
| 19 | 六曜・旧暦表示（qreki_dart、月カレンダーセル＆選択日バー表示、設定ON/OFF） | ✅ 完了 |
| 20 | プレゼント・お祝い履歴管理（DB v8、年度別ギフト履歴、金額・メモ、JSONバックアップ連携） | ✅ 完了 |
| 21 | 操作性向上（既存予定の複製作成、月選択ジャンプシート、カレンダー初期値設定） | ✅ 完了 |
| 22 | プレミアム・マネタイズ（写真きせかえ・拡張ウィジェット・isPremiumProvider・Paywall・IAP連携） | 📋 計画中 |

**全体進捗: コア機能・拡張機能完了、フェーズ22（マネタイズ基盤）計画中** — 全44テスト通過完了済み
...
| 設定機能 | ✅ 実装済み | 基本設定・カレンダー設定・誕生日表示設定・ダークモード設定 |
| タグ管理 | ✅ 実装済み | タグ一覧・追加・削除・誕生日紐付け |
| スタンプ・アイコン | ✅ 実装済み | 予定アイコン・スタンプのカレンダー表示・クイック追加 |
| データバックアップ | ✅ 実装済み | JSONエクスポート・共有・上書き/追加復元機能・.ics書き出し |
...
| パスコードロック・生体認証 | ✅ 実装済み | 4桁PIN、SHA-256暗号化、指紋/顔認証、目隠しシールド |
| 六曜・旧暦表示 | ✅ 実装済み | 大安・友引・仏滅などのカレンダー表示・設定ON/OFF |
| プレゼント履歴メモ | ✅ 実装済み | 誕生日ごとのプレゼント・お祝い履歴の記録（あげた/もらった/候補） |
| 予定の複製（コピー作成） | ✅ 実装済み | 予定詳細から内容を引き継いだ複製モーダル起動 |
| 年月ジャンプ | ✅ 実装済み | ヘッダータップからの月選択シートによる高速年月移動 |
| テスト | ✅ 実装済み | Unit/Widget テスト（全44件パス） |
| 写真きせかえ（プレミアム） | 計画中 (フェーズ22-1) | フォトライブラリから好きな写真をカレンダー背景に設定（推し活・子供・ペット） |
| 拡張ウィジェット（プレミアム） | 計画中 (フェーズ22-1) | 写真付きカウントダウン、背景透過、カスタムカラーウィジェット |
| プレミアム状態管理・Paywall | 計画中 (フェーズ22-2,3) | isPremiumProvider、デバッグ用切り替えスイッチ、料金案内モーダル |
| 課金決済・広告連携 | 計画中 (フェーズ22-4) | RevenueCat (purchases_flutter) 決済、Google AdMob 連携 |
| 記念日への拡張 | 未実装 (候補) | 結婚記念日・命日・ペット記念日などのカウントダウン管理 |
| 予定の定型テンプレート | 未実装 (候補) | シフトや通院など、よく使う予定のワンタップ入力 |
| お祝い連絡ショートカット | 未実装 (候補) | 誕生日詳細からのLINE/電話/SMS直接起動 |
| 干支・星座・長寿祝い表示 | 未実装 (候補) | 生年月日から干支・星座・還暦等の長寿祝い・厄年自動表示 |
| クラウド自動バックアップ | 未実装 (候補) | Google Drive / iCloud への自動同期 |
